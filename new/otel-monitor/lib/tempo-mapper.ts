...
export function mapTempoTrace(raw: RawTraceData): Trace | null {
  ...
  // Only count MCP-server-side spans. Openinference TOOL spans
  // (identified by `tool.name` attr) describe the orchestrator-side
  // wrapper for the same invocation and would double the count.
  rootAttrs['agent.tool_count']               = children.filter(c =>
    c.name.includes('_operation') || c.name.startsWith('mcp.')).length;
